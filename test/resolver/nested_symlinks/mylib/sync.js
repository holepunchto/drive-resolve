const a = require.resolve('buffer/').replace(process.cwd(), '$CWD')
const b = require('resolve').sync('buffer/', { preserveSymlinks: true }).replace(process.cwd(), '$CWD')
const c = require('resolve').sync('buffer/', { preserveSymlinks: false }).replace(process.cwd(), '$CWD')
